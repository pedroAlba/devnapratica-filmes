...
public class ListFilmeImpl {
	...
	public ListFilmeImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(Filme.PageRequest input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				BasicoConstants.DOMAIN, //
				BasicoConstants.SERVICE, //
				BasicoConstants.Commands.LIST_FILME, //
	...
	*/
	public Filme.PagedResults listFilme(Filme.PageRequest input, long timeout) {
		...
		Filme.PagedResults output = DtoJsonConverter.toDTO(resultMessage.getContent(), Filme.PagedResults.class);
		if (output == null) {
	...
	*/
	public CompletableFuture<Filme.PagedResults> listFilmeRequest(Filme.PageRequest input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, Filme.PagedResults.class);
	}
