 ...
 * 
 */
public class CreateMergeGeneroImpl {
	...
	public CreateMergeGeneroImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(Genero input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				BasicoConstants.DOMAIN, //
				BasicoConstants.SERVICE, //
				BasicoConstants.Commands.CREATE_MERGE_GENERO, //
	...
	*/
	public Genero createMergeGenero(Genero input, long timeout) {
		...
		Genero output = DtoJsonConverter.toDTO(resultMessage.getContent(), Genero.class);
		if (output == null) {
	...
	*/
	public CompletableFuture<Genero> createMergeGeneroRequest(Genero input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, Genero.class);
	}
