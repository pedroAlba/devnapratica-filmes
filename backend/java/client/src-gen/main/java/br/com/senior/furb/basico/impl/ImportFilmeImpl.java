...
public class ImportFilmeImpl {
	...
	public ImportFilmeImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(ImportFilmeInput input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				BasicoConstants.DOMAIN, //
				BasicoConstants.SERVICE, //
				BasicoConstants.Commands.IMPORT_FILME, //
	...
	*/
	public ImportFilmeOutput importFilme(ImportFilmeInput input, long timeout) {
		...
		ImportFilmeOutput output = DtoJsonConverter.toDTO(resultMessage.getContent(), ImportFilmeOutput.class);
		if (output == null) {
	...
	*/
	public CompletableFuture<ImportFilmeOutput> importFilmeRequest(ImportFilmeInput input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, ImportFilmeOutput.class);
	}
