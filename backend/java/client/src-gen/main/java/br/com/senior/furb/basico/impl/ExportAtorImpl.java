...
public class ExportAtorImpl {
	...
	public ExportAtorImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(ExportAtorInput input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				BasicoConstants.DOMAIN, //
				BasicoConstants.SERVICE, //
				BasicoConstants.Commands.EXPORT_ATOR, //
	...
	*/
	public ExportAtorOutput exportAtor(ExportAtorInput input, long timeout) {
		...
		ExportAtorOutput output = DtoJsonConverter.toDTO(resultMessage.getContent(), ExportAtorOutput.class);
		if (output == null) {
	...
	*/
	public CompletableFuture<ExportAtorOutput> exportAtorRequest(ExportAtorInput input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, ExportAtorOutput.class);
	}
