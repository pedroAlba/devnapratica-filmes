 ...
 * 
 */
public class CreateMergeAtorImpl {
	...
	public CreateMergeAtorImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(Ator input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				BasicoConstants.DOMAIN, //
				BasicoConstants.SERVICE, //
				BasicoConstants.Commands.CREATE_MERGE_ATOR, //
	...
	*/
	public Ator createMergeAtor(Ator input, long timeout) {
		...
		Ator output = DtoJsonConverter.toDTO(resultMessage.getContent(), Ator.class);
		if (output == null) {
	...
	*/
	public CompletableFuture<Ator> createMergeAtorRequest(Ator input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, Ator.class);
	}
