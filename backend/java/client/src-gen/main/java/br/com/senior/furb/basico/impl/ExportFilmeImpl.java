...
public class ExportFilmeImpl {
	...
	public ExportFilmeImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(ExportFilmeInput input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				BasicoConstants.DOMAIN, //
				BasicoConstants.SERVICE, //
				BasicoConstants.Commands.EXPORT_FILME, //
	...
	*/
	public ExportFilmeOutput exportFilme(ExportFilmeInput input, long timeout) {
		...
		ExportFilmeOutput output = DtoJsonConverter.toDTO(resultMessage.getContent(), ExportFilmeOutput.class);
		if (output == null) {
	...
	*/
	public CompletableFuture<ExportFilmeOutput> exportFilmeRequest(ExportFilmeInput input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, ExportFilmeOutput.class);
	}
