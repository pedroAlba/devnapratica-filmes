 ...
 * 
 */
public class ImportDiretorImpl {
	...
	public ImportDiretorImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(ImportDiretorInput input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				BasicoConstants.DOMAIN, //
				BasicoConstants.SERVICE, //
				BasicoConstants.Commands.IMPORT_DIRETOR, //
	...
	*/
	public ImportDiretorOutput importDiretor(ImportDiretorInput input, long timeout) {
		...
		ImportDiretorOutput output = DtoJsonConverter.toDTO(resultMessage.getContent(), ImportDiretorOutput.class);
		if (output == null) {
	...
	*/
	public CompletableFuture<ImportDiretorOutput> importDiretorRequest(ImportDiretorInput input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, ImportDiretorOutput.class);
	}
