...
public class ListDiretorImpl {
	...
	public ListDiretorImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(Diretor.PageRequest input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				BasicoConstants.DOMAIN, //
				BasicoConstants.SERVICE, //
				BasicoConstants.Commands.LIST_DIRETOR, //
	...
	*/
	public Diretor.PagedResults listDiretor(Diretor.PageRequest input, long timeout) {
		...
		Diretor.PagedResults output = DtoJsonConverter.toDTO(resultMessage.getContent(), Diretor.PagedResults.class);
		if (output == null) {
	...
	*/
	public CompletableFuture<Diretor.PagedResults> listDiretorRequest(Diretor.PageRequest input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, Diretor.PagedResults.class);
	}
