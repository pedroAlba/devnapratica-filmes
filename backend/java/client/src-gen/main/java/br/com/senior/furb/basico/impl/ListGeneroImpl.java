...
public class ListGeneroImpl {
	...
	public ListGeneroImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(Genero.PageRequest input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				BasicoConstants.DOMAIN, //
				BasicoConstants.SERVICE, //
				BasicoConstants.Commands.LIST_GENERO, //
	...
	*/
	public Genero.PagedResults listGenero(Genero.PageRequest input, long timeout) {
		...
		Genero.PagedResults output = DtoJsonConverter.toDTO(resultMessage.getContent(), Genero.PagedResults.class);
		if (output == null) {
	...
	*/
	public CompletableFuture<Genero.PagedResults> listGeneroRequest(Genero.PageRequest input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, Genero.PagedResults.class);
	}
