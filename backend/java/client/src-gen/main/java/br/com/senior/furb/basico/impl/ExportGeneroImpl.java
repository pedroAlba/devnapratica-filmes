...
public class ExportGeneroImpl {
	...
	public ExportGeneroImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(ExportGeneroInput input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				BasicoConstants.DOMAIN, //
				BasicoConstants.SERVICE, //
				BasicoConstants.Commands.EXPORT_GENERO, //
	...
	*/
	public ExportGeneroOutput exportGenero(ExportGeneroInput input, long timeout) {
		...
		ExportGeneroOutput output = DtoJsonConverter.toDTO(resultMessage.getContent(), ExportGeneroOutput.class);
		if (output == null) {
	...
	*/
	public CompletableFuture<ExportGeneroOutput> exportGeneroRequest(ExportGeneroInput input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, ExportGeneroOutput.class);
	}
