...
public class ImportGeneroImpl {
	...
	public ImportGeneroImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(ImportGeneroInput input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				BasicoConstants.DOMAIN, //
				BasicoConstants.SERVICE, //
				BasicoConstants.Commands.IMPORT_GENERO, //
	...
	*/
	public ImportGeneroOutput importGenero(ImportGeneroInput input, long timeout) {
		...
		ImportGeneroOutput output = DtoJsonConverter.toDTO(resultMessage.getContent(), ImportGeneroOutput.class);
		if (output == null) {
	...
	*/
	public CompletableFuture<ImportGeneroOutput> importGeneroRequest(ImportGeneroInput input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, ImportGeneroOutput.class);
	}
