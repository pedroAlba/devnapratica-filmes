...
public class ImportAtorImpl {
	...
	public ImportAtorImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(ImportAtorInput input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				BasicoConstants.DOMAIN, //
				BasicoConstants.SERVICE, //
				BasicoConstants.Commands.IMPORT_ATOR, //
	...
	*/
	public ImportAtorOutput importAtor(ImportAtorInput input, long timeout) {
		...
		ImportAtorOutput output = DtoJsonConverter.toDTO(resultMessage.getContent(), ImportAtorOutput.class);
		if (output == null) {
	...
	*/
	public CompletableFuture<ImportAtorOutput> importAtorRequest(ImportAtorInput input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, ImportAtorOutput.class);
	}
