 ...
 * 
 */
public class UpdateMergeDiretorImpl {
	...
	public UpdateMergeDiretorImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(Diretor input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				BasicoConstants.DOMAIN, //
				BasicoConstants.SERVICE, //
				BasicoConstants.Commands.UPDATE_MERGE_DIRETOR, //
	...
	*/
	public Diretor updateMergeDiretor(Diretor input, long timeout) {
		...
		Diretor output = DtoJsonConverter.toDTO(resultMessage.getContent(), Diretor.class);
		if (output == null) {
	...
	*/
	public CompletableFuture<Diretor> updateMergeDiretorRequest(Diretor input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, Diretor.class);
	}
