 ...
 * 
 */
public class CreateMergeFilmeImpl {
	...
	public CreateMergeFilmeImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(Filme input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				BasicoConstants.DOMAIN, //
				BasicoConstants.SERVICE, //
				BasicoConstants.Commands.CREATE_MERGE_FILME, //
	...
	*/
	public Filme createMergeFilme(Filme input, long timeout) {
		...
		Filme output = DtoJsonConverter.toDTO(resultMessage.getContent(), Filme.class);
		if (output == null) {
	...
	*/
	public CompletableFuture<Filme> createMergeFilmeRequest(Filme input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, Filme.class);
	}
